...
/**
 * Barra de filtros de Ingresos: chips de moneda (Todos/Soles/Dólares) +
 * dropdowns de banco y mes. Componente presentacional puro (patrón
 * `v-model`/emit, igual que `FiltrosHistorial`): no calcula el filtrado, solo
 * emite la selección para que la vista la aplique sobre `store.ingresos`.
 * A diferencia de `FiltrosHistorial`, no tiene filtro de categoría porque
 * `Ingreso` no tiene ese concepto en el modelo de datos.
 */
